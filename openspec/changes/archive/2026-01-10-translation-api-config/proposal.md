# Proposal: translation-api-config

## Summary

新增翻译 API 配置功能，支持配置多个翻译服务（有道翻译、百度翻译、谷歌翻译等），采用与 AI 节点类似的配置方式，支持多节点、优先级排序、连通性测试和自动故障转移。

## Why

当前翻译功能存在以下问题：

1. **硬编码依赖**：词汇查询依赖 `dict.youdao.com`，语句翻译依赖 `fanyi.youdao.com`，无法自定义
2. **单点故障**：有道 API 不可用时无法自动切换到其他服务
3. **灵活性不足**：用户无法根据自己的需求选择翻译服务（如需要谷歌翻译的质量或百度翻译的中文优化）
4. **成本控制**：无法利用用户已有的翻译 API 配额

通过引入可配置的翻译 API 节点，用户可以：
- 使用自己的 API 密钥，避免共享配额限制
- 配置多个翻译服务作为备选
- 根据翻译质量和速度选择最适合的服务

## Scope

### In Scope

- 翻译 API 节点配置 UI（参考现有 AI 节点配置）
- 支持的翻译服务：有道翻译、百度翻译、谷歌翻译、DeepL
- 节点管理：添加、编辑、删除、排序（优先级）
- 连通性测试功能
- 速率限制配置（可选）
- 自动故障转移机制
- 降级策略：无配置时使用现有免费 API

### Out of Scope

- 自定义翻译 API 端点（非标准 API）
- 翻译结果缓存策略调整（使用现有缓存）
- 翻译质量评估和自动选择
- 付费服务的计费管理

## Solution Overview

### 配置 UI 设计

在设置页面「API 配置」section 下新增「翻译 API」子区域，与现有 AI 节点配置并列：

```
┌─────────────────────────────────────────────────────────┐
│ API 配置                                                │
├─────────────────────────────────────────────────────────┤
│ ▼ AI 节点                                               │
│   [现有 AI 节点配置...]                                  │
│                                                         │
│ ▼ 翻译 API                                              │
│   ┌─────────────────────────────────────────────────┐   │
│   │ 翻译 API 节点                                    │   │
│   │ 拖拽调整优先级，优先使用排在前面的节点            │   │
│   │                                    [+ 添加节点]  │   │
│   ├─────────────────────────────────────────────────┤   │
│   │ ┌─────────────────────────────────────────────┐ │   │
│   │ │ ≡ 百度翻译                        ✓ 已连接  │ │   │
│   │ │   appid: 2024***                           │ │   │
│   │ │   [编辑] [测试] [删除]                      │ │   │
│   │ └─────────────────────────────────────────────┘ │   │
│   │ ┌─────────────────────────────────────────────┐ │   │
│   │ │ ≡ 有道翻译                        ○ 未测试  │ │   │
│   │ │   appKey: abc***                           │ │   │
│   │ │   [编辑] [测试] [删除]                      │ │   │
│   │ └─────────────────────────────────────────────┘ │   │
│   └─────────────────────────────────────────────────┘   │
│                                                         │
│   ⓘ 未配置翻译 API 时，将使用内置免费服务（有限制）      │
└─────────────────────────────────────────────────────────┘
```

### 节点编辑弹窗

```
┌─────────────────────────────────────────────────────────┐
│ 添加翻译节点                                      [×]   │
├─────────────────────────────────────────────────────────┤
│ 节点名称                                                │
│ [百度翻译                                          ]    │
│                                                         │
│ 翻译服务                                                │
│ [▼ 百度翻译                                       ]    │
│                                                         │
│ APP ID / App Key                                        │
│ [20241234567890                                    ]    │
│                                                         │
│ 密钥 / Secret Key                                       │
│ [********************************              👁 ]    │
│                                                         │
│ 速率限制 (QPS)                                          │
│ [10        ] 次/秒    ⓘ 0 或留空表示无限制              │
│                                                         │
│ 快速填充                                                │
│ [百度翻译] [有道翻译] [谷歌翻译] [DeepL]                 │
├─────────────────────────────────────────────────────────┤
│ [✓ 测试连接]  连接成功 ✓                                │
│                                    [取消]  [保存]       │
└─────────────────────────────────────────────────────────┘
```

### 支持的翻译服务

| 服务 | API 类型 | 认证方式 | 特点 | 免费额度 |
|------|----------|----------|------|----------|
| 谷歌翻译 | REST | API Key | 多语言质量高 | 50万字符/月 |
| 百度翻译 | REST | appid + secretKey + sign | 中文优化 | 100万字符/月（个人认证） |
| 腾讯云翻译 | REST | SecretId + SecretKey + sign | 免费额度最大 | 500万字符/月 |
| 有道智云 | REST | appKey + appSecret + sign | 词典功能强，音标支持 | 一次性50元额度 |
| DeepL | REST | API Key | 翻译质量最佳 | 50万字符/月（Free版） |

### 数据结构

```javascript
// 存储结构
{
  translationNodes: [
    {
      id: "uuid-1",
      name: "百度翻译",
      provider: "baidu",        // google | baidu | tencent | youdao | deepl
      appId: "20241234567890",  // 百度/有道
      secretId: "",             // 腾讯云
      secretKey: "encrypted",   // 加密存储
      apiKey: "",               // 谷歌/DeepL
      rateLimit: 10,            // QPS，0 表示无限制
      enabled: true,
      lastTestTime: 1704067200000,
      lastTestResult: "success" // success | failed | untested
    }
  ],
  translationFallbackEnabled: true  // 无节点时是否使用内置服务
}
```

### 翻译策略

```
用户选中文本
    ↓
检查是否有配置的翻译节点
    ↓
┌─────────────────────────────────────────┐
│ 有配置节点                               │
│   ↓                                     │
│ 按优先级依次尝试                         │
│   ↓                                     │
│ 节点可用？ ──否──→ 尝试下一节点          │
│   ↓是                                   │
│ 调用翻译 API                            │
│   ↓                                     │
│ 成功？ ──否──→ 标记节点失败，尝试下一节点 │
│   ↓是                                   │
│ 返回结果                                │
└─────────────────────────────────────────┘
    ↓ 所有节点失败或无配置
┌─────────────────────────────────────────┐
│ 降级处理                                 │
│   ↓                                     │
│ fallbackEnabled？                       │
│   ↓是                                   │
│ 使用内置免费服务（现有逻辑）              │
│   ↓否                                   │
│ 返回错误提示                            │
└─────────────────────────────────────────┘
```

### 统一适配层架构

由于各翻译 API 提供商的参数格式和返回值结构各不相同，需要设计统一的适配层来屏蔽差异。

#### 架构图

```
┌─────────────────────────────────────────────────────────────────┐
│                        上层调用方                                │
│              (api-client.js / event-handlers.js)                │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    TranslationService                           │
│                      (统一入口)                                  │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │ translate(request: TranslationRequest): TranslationResult │  │
│  │ lookup(word: string): DictionaryResult                    │  │
│  │ testConnection(nodeId): boolean                           │  │
│  └───────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    ProviderAdapter                              │
│                   (适配器基类/接口)                              │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │ + buildRequest(req: TranslationRequest): ProviderRequest  │  │
│  │ + parseResponse(resp: any): TranslationResult             │  │
│  │ + mapLanguageCode(code: string): string                   │  │
│  │ + getCapabilities(): ProviderCapabilities                 │  │
│  └───────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
                              │
        ┌─────────────────────┼─────────────────────┬─────────────────────┐
        ▼                     ▼                     ▼                     ▼
┌───────────────┐   ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
│ GoogleAdapter │   │ BaiduAdapter  │   │TencentAdapter │   │ YoudaoAdapter │  ...
│               │   │               │   │               │   │               │
│ - API Key    │   │ - MD5 签名    │   │ - HMAC 签名   │   │ - SHA256 签名 │
│ - 语言映射    │   │ - 语言映射    │   │ - 语言映射    │   │ - 词典解析    │
│ - 结果转换    │   │ - 结果转换    │   │ - 结果转换    │   │ - 音标提取    │
└───────────────┘   └───────────────┘   └───────────────┘   └───────────────┘
```

#### 统一数据结构

```typescript
// ==================== 输入结构 ====================

/** 翻译请求（统一格式） */
interface TranslationRequest {
  text: string;              // 待翻译文本
  from: LanguageCode;        // 源语言（统一编码）
  to: LanguageCode;          // 目标语言（统一编码）
  type: 'word' | 'sentence'; // 翻译类型
  options?: {
    needPhonetic?: boolean;  // 是否需要音标
    needDict?: boolean;      // 是否需要词典释义
    needExample?: boolean;   // 是否需要例句
  };
}

/** 统一语言编码（ISO 639-1 为基础） */
type LanguageCode =
  | 'zh-CN'  // 简体中文
  | 'zh-TW'  // 繁体中文
  | 'en'     // 英语
  | 'ja'     // 日语
  | 'ko'     // 韩语
  | 'fr'     // 法语
  | 'de'     // 德语
  | 'es'     // 西班牙语
  | 'auto';  // 自动检测

// ==================== 输出结构 ====================

/** 翻译结果（统一格式） */
interface TranslationResult {
  success: boolean;
  provider: string;          // 实际使用的提供商

  // 基础翻译
  translation: string;       // 翻译结果
  detectedLang?: LanguageCode; // 检测到的源语言

  // 词典信息（单词翻译时）
  dictionary?: DictionaryInfo;

  // 错误信息
  error?: {
    code: string;
    message: string;
  };
}

/** 词典信息（统一格式） */
interface DictionaryInfo {
  phonetic?: {
    us?: string;             // 美式音标
    uk?: string;             // 英式音标
  };
  definitions?: Array<{
    pos: string;             // 词性 (n. / v. / adj. ...)
    meanings: string[];      // 释义列表
    examples?: string[];     // 例句
  }>;
  forms?: {                  // 词形变化
    plural?: string;
    past?: string;
    pastParticiple?: string;
    presentParticiple?: string;
  };
}

// ==================== 提供商能力声明 ====================

/** 提供商能力 */
interface ProviderCapabilities {
  supportedLanguages: LanguageCode[];  // 支持的语言
  supportDict: boolean;                // 是否支持词典
  supportPhonetic: boolean;            // 是否支持音标
  supportDetect: boolean;              // 是否支持语言检测
  maxTextLength: number;               // 最大文本长度
  qpsLimit: number;                    // 默认 QPS 限制
}
```

#### 语言代码映射

各提供商使用不同的语言代码，适配器负责转换：

| 统一编码 | 谷歌 | 百度 | 腾讯云 | 有道 | DeepL |
|----------|------|------|--------|------|-------|
| `zh-CN`  | `zh-CN` | `zh` | `zh` | `zh-CHS` | `ZH` |
| `zh-TW`  | `zh-TW` | `cht` | `zh-TW` | `zh-CHT` | `ZH` |
| `en`     | `en` | `en` | `en` | `en` | `EN` |
| `ja`     | `ja` | `jp` | `ja` | `ja` | `JA` |
| `ko`     | `ko` | `kor` | `ko` | `ko` | `KO` |
| `fr`     | `fr` | `fra` | `fr` | `fr` | `FR` |
| `de`     | `de` | `de` | `de` | `de` | `DE` |
| `auto`   | `auto` | `auto` | `auto` | `auto` | ❌ |

#### 适配器实现示例

```javascript
// translation-adapters.js

/**
 * 适配器基类
 */
class BaseAdapter {
  constructor(config) {
    this.config = config;
  }

  // 子类必须实现
  async doTranslate(text, from, to) {
    throw new Error('Not implemented');
  }

  // 子类必须实现
  mapLanguageCode(code) {
    throw new Error('Not implemented');
  }

  // 子类可选实现
  parseDict(response) {
    return null;
  }

  // 统一入口
  async translate(request) {
    try {
      const from = this.mapLanguageCode(request.from);
      const to = this.mapLanguageCode(request.to);

      const rawResponse = await this.doTranslate(request.text, from, to);

      return {
        success: true,
        provider: this.name,
        translation: this.extractTranslation(rawResponse),
        dictionary: request.options?.needDict
          ? this.parseDict(rawResponse)
          : undefined
      };
    } catch (error) {
      return {
        success: false,
        provider: this.name,
        translation: '',
        error: {
          code: error.code || 'UNKNOWN',
          message: error.message
        }
      };
    }
  }
}

/**
 * 百度翻译适配器
 */
class BaiduAdapter extends BaseAdapter {
  name = 'baidu';

  languageMap = {
    'zh-CN': 'zh', 'zh-TW': 'cht', 'en': 'en',
    'ja': 'jp', 'ko': 'kor', 'fr': 'fra',
    'de': 'de', 'es': 'spa', 'auto': 'auto'
  };

  mapLanguageCode(code) {
    return this.languageMap[code] || code;
  }

  async doTranslate(text, from, to) {
    const { appId, secretKey } = this.config;
    const salt = Date.now();
    const sign = md5(appId + text + salt + secretKey);

    const response = await fetch(
      `https://fanyi-api.baidu.com/api/trans/vip/translate?` +
      new URLSearchParams({ q: text, from, to, appid: appId, salt, sign })
    );

    const data = await response.json();
    if (data.error_code) {
      throw { code: data.error_code, message: data.error_msg };
    }
    return data;
  }

  extractTranslation(response) {
    return response.trans_result?.[0]?.dst || '';
  }

  getCapabilities() {
    return {
      supportedLanguages: ['zh-CN', 'zh-TW', 'en', 'ja', 'ko', 'fr', 'de', 'es'],
      supportDict: false,
      supportPhonetic: false,
      supportDetect: true,
      maxTextLength: 6000,
      qpsLimit: 10
    };
  }
}

/**
 * 有道翻译适配器（支持词典）
 */
class YoudaoAdapter extends BaseAdapter {
  name = 'youdao';

  languageMap = {
    'zh-CN': 'zh-CHS', 'zh-TW': 'zh-CHT', 'en': 'en',
    'ja': 'ja', 'ko': 'ko', 'fr': 'fr',
    'de': 'de', 'es': 'es', 'auto': 'auto'
  };

  mapLanguageCode(code) {
    return this.languageMap[code] || code;
  }

  async doTranslate(text, from, to) {
    const { appKey, appSecret } = this.config;
    const salt = Date.now().toString();
    const curtime = Math.floor(Date.now() / 1000).toString();
    const input = text.length > 20
      ? text.slice(0, 10) + text.length + text.slice(-10)
      : text;
    const sign = sha256(appKey + input + salt + curtime + appSecret);

    const response = await fetch('https://openapi.youdao.com/api', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
        q: text, from, to, appKey, salt, sign, signType: 'v3', curtime
      })
    });

    const data = await response.json();
    if (data.errorCode !== '0') {
      throw { code: data.errorCode, message: this.getErrorMessage(data.errorCode) };
    }
    return data;
  }

  extractTranslation(response) {
    return response.translation?.[0] || '';
  }

  // 有道特有：解析词典信息
  parseDict(response) {
    if (!response.basic) return null;

    const basic = response.basic;
    return {
      phonetic: {
        us: basic['us-phonetic'],
        uk: basic['uk-phonetic']
      },
      definitions: this.parseExplains(basic.explains)
    };
  }

  parseExplains(explains) {
    if (!explains) return [];
    return explains.map(exp => {
      const match = exp.match(/^([a-z]+\.)\s*(.+)$/);
      if (match) {
        return { pos: match[1], meanings: [match[2]] };
      }
      return { pos: '', meanings: [exp] };
    });
  }

  getCapabilities() {
    return {
      supportedLanguages: ['zh-CN', 'zh-TW', 'en', 'ja', 'ko', 'fr', 'de', 'es'],
      supportDict: true,
      supportPhonetic: true,
      supportDetect: true,
      maxTextLength: 5000,
      qpsLimit: 10
    };
  }
}

// 其他适配器类似实现...
```

#### 翻译服务统一入口

```javascript
// translation-service.js

class TranslationService {
  constructor() {
    this.adapters = {
      google: GoogleAdapter,
      baidu: BaiduAdapter,
      tencent: TencentAdapter,
      youdao: YoudaoAdapter,
      deepl: DeepLAdapter
    };
  }

  /**
   * 统一翻译入口
   * @param {TranslationRequest} request
   * @returns {Promise<TranslationResult>}
   */
  async translate(request) {
    const nodes = await this.getEnabledNodes();

    // 按优先级依次尝试
    for (const node of nodes) {
      const adapter = this.createAdapter(node);
      const result = await adapter.translate(request);

      if (result.success) {
        return result;
      }

      // 记录失败，继续尝试下一个
      console.warn(`[Translation] ${node.name} failed:`, result.error);
    }

    // 所有节点失败，尝试降级
    if (await this.isFallbackEnabled()) {
      return this.fallbackTranslate(request);
    }

    return {
      success: false,
      provider: 'none',
      translation: '',
      error: { code: 'ALL_FAILED', message: '所有翻译服务均不可用' }
    };
  }

  /**
   * 词典查询（优先使用支持词典的服务）
   */
  async lookup(word) {
    const nodes = await this.getEnabledNodes();

    // 优先选择支持词典的节点
    const dictNodes = nodes.filter(n =>
      this.adapters[n.provider]?.prototype?.getCapabilities?.()?.supportDict
    );

    const targetNodes = dictNodes.length > 0 ? dictNodes : nodes;

    return this.translate({
      text: word,
      from: 'auto',
      to: 'zh-CN',
      type: 'word',
      options: { needDict: true, needPhonetic: true }
    });
  }

  createAdapter(node) {
    const AdapterClass = this.adapters[node.provider];
    if (!AdapterClass) {
      throw new Error(`Unknown provider: ${node.provider}`);
    }
    return new AdapterClass(node);
  }
}

// 导出单例
const translationService = new TranslationService();
```

### 技术实现

#### 1. 新增文件

| 文件 | 职责 |
|------|------|
| `js/content/translation-service.js` | 翻译服务统一入口 |
| `js/content/translation-adapters.js` | 各提供商适配器实现 |
| `js/content/translation-types.js` | 类型定义和常量 |
| `js/options-translation.js` | 翻译节点配置 UI 逻辑 |

#### 2. 修改文件

| 文件 | 改动 |
|------|------|
| `options.html` | 添加翻译 API 配置 UI |
| `css/options.css` | 翻译节点样式（复用 AI 节点样式） |
| `js/content/api-client.js` | 调用 `translationService.translate()` |
| `js/content/storage.js` | 添加翻译节点存储逻辑 |
| `manifest.json` | 添加新 JS 文件引用 |

## Impact

### 用户体验
- 可使用自己的 API 密钥，避免共享限制
- 多节点备选，提高翻译可用性
- 可选择翻译质量更高的服务

### 代码改动
- 中等规模，主要是新增配置 UI 和 API 封装
- 复用现有 AI 节点配置的 UI 模式和样式
- 对现有翻译逻辑的改动较小（仅修改调用入口）

### 兼容性
- 完全向后兼容，未配置时使用现有逻辑
- 现有用户无需任何操作即可继续使用

## Risks & Mitigations

| 风险 | 缓解措施 |
|------|----------|
| API 密钥泄露 | 加密存储，UI 隐藏显示 |
| 各服务 API 变更 | 模块化封装，易于更新 |
| 用户配置错误 | 连通性测试 + 详细错误提示 |
| 免费额度用尽 | 显示剩余额度（如 API 支持） |
| 跨域请求限制 | 通过 background script 代理请求 |

## Dependencies

- 现有 AI 节点配置 UI 组件（复用）
- MD5 签名库（百度/有道 API 需要）
- 各翻译服务的 API 文档

## Related Specs

- `enhance-selection-popup` - 使用翻译功能的上层功能
- `translation-config` - 现有翻译配置（将被扩展）

## Status

- [x] Proposal drafted
- [ ] Design reviewed
- [ ] Tasks defined
- [ ] Approved for implementation
