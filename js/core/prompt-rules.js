...
// 核心提示词（系统固定）
export const CORE_PROMPT = `你是一个语言学习助手。请分析以下文本，选择适合学习的词汇进行翻译。
...
// 源语言规则（根据源语言动态注入）
export const SOURCE_LANGUAGE_RULES = {
  'zh-CN': `
## 中文语义分词规则：
- 按语义边界识别词汇，而非机械切分
...
// 目标语言规则（根据目标语言动态注入）
export const TARGET_LANGUAGE_RULES = {
  'zh-CN': `
## 中文输出规则：
- 使用简体中文
- 翻译应自然流畅，符合中文表达习惯
 ...
 * @param {number} options.targetCount - 目标词汇数量
 * @param {number} options.maxCount - 最大词汇数量
 * @param {string} options.customPrompt - 用户自定义提示词（可选）
...
export function buildTranslationPrompt(options) {
  ...
  // 获取源语言规则（支持语言代码前缀匹配）
  const sourceRule = SOURCE_LANGUAGE_RULES[sourceLang] ||
                     SOURCE_LANGUAGE_RULES[sourceLang.split('-')[0]] ||
                     '';

  // 获取目标语言规则
  const targetRule = TARGET_LANGUAGE_RULES[targetLang] ||
...
- original: 原词
- translation: 翻译结果
- phonetic: 学习语言(${targetLang})的音标/发音
 ...
 * @param {string} options.sourceLang - 源语言代码
 * @param {string} options.targetLang - 目标语言代码
 * @param {string[]} options.words - 待翻译词汇列表
 * @param {string} options.customPrompt - 用户自定义提示词（可选）
 * @returns {string} 完整的提示词
 */
export function buildSpecificWordsPrompt(options) {
  const {
    sourceLang,
    targetLang,
    words,
 ...
 * @param {string} options.sourceLang - 源语言代码
 * @param {string} options.targetLang - 目标语言代码
 * @param {string} options.word - 待翻译词汇
 * @param {string} options.context - 上下文句子
 * @param {string} options.customPrompt - 用户自定义提示词（可选）
 * @returns {string} 完整的提示词
 */
export function buildRetranslatePrompt(options) {
  const {
    sourceLang,
    targetLang,
    word,
