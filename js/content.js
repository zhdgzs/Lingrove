...
- Adaptar el registro lingüístico`
  ...
  function buildTranslationPrompt(options) {
    const { sourceLang, targetLang, text, targetCount, maxCount, customPrompt } = options;

    // 获取源语言规则（优先使用自定义规则）
    const defaultSourceRule = SOURCE_LANGUAGE_RULES[sourceLang] ||
...
只返回 JSON 数组，不要其他内容。`;
   ...
   * 构建特定词汇翻译的提示词
   */
  function buildSpecificWordsPrompt(options) {
    const { sourceLang, targetLang, words, customPrompt } = options;
    const userRule = customPrompt || '';
...
只返回 JSON 数组，不要其他内容。`;
   ...
   * 构建上下文重新翻译的提示词
   */
  function buildRetranslatePrompt(options) {
    const { sourceLang, targetLang, word, context, customPrompt } = options;
    const userRule = customPrompt || '';
...
只返回 JSON，不要其他内容。`;
      ...
      chrome.storage.sync.get(null, (syncResult) => {
        ...
        chrome.storage.local.get(['learnedWords', 'memorizeList'], (localResult) => {
          ...
          config = {
            ...
            customPromptRules: syncResult.customPromptRules || DEFAULT_CUSTOM_PROMPT,
    ...
    const asyncPromise = (async () => {
      try {
        ...
        const prompt = buildTranslationPrompt({
          ...
          customPrompt: config.customPromptRules
        });
  ...
  async function translateSpecificWords(targetWords) {
    ...
    if (uncached.length > 0) {
      try {
        // 使用动态提示词构建函数
        const prompt = buildSpecificWordsPrompt({
          sourceLang,
          targetLang,
          words: uncached,
          customPrompt: config.customPromptRules
